...
// ------ADD：测量时间MeasureTime需要格式化为"2006-01-02 15:04:05.000000"------
std::string getFormattedMeasureTime()
{
    struct timeval tv;
    gettimeofday(&tv, NULL);
    char buffer[26];
    ...
    char microseconds[7];
    snprintf(microseconds, 7, ".%06d", tv.tv_usec);
    // sprintf(microseconds, ".%06ld", tv.tv_usec);
    return std::string(buffer) + std::string(microseconds);
}

// ------ADD：计算时间差，输出为以秒为单位，s结尾的字符串"------

double getTimeAsSeconds(const std::string &time) {
    std::tm tm = {};
    int microsec;
    ...
    ss.ignore();
    ss >> microsec;
    std::chrono::seconds sec = std::chrono::seconds(mktime(&tm));
    std::chrono::microseconds usec = std::chrono::microseconds(microsec);
    return (sec.count() + usec.count() * 1e-6);
}
std::string calcTimeDiff(const std::string &time1, const std::string &time2) {
    double t1 = getTimeAsSeconds(time1);
    double t2 = getTimeAsSeconds(time2);
    double diff = t2 - t1;
    std::stringstream ss;
    ss << std::fixed << std::setprecision(6) << diff << "s";
    return ss.str();
}
