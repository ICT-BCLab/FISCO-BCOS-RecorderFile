...
ImportResult TxPool::import(Transaction::Ptr _tx, IfDropped)
{
    ...
    auto tx_hash=_tx->hash(); // 交易哈希
    auto source=std::to_string(1); // 交易来源 1表示本地交易
    if(_tx->rpcTx()){
        source=std::to_string(2); // rpc交易
    }
    // record交易池输入通量
    stringstream ss;
    ss<<measure_time<<","<<tx_hash<<","<<source<<"\n";
    std::shared_ptr<RecorderFile> recorderfile(new RecorderFile());
    recorderfile->Record(ss.str(),"transaction_pool_input_throughput");
    ...
    std::shared_ptr<dev::eth::Block> _block, size_t _index)
{
    ...
    std::unordered_map<h256, TransactionQueue::iterator>::iterator p_tx;
    {
        ...
        // ---交易离开了交易池---
        auto measure_time=dev::getFormattedMeasureTime(); // 测量时间
        auto tx_hash=_txHash;
        auto flag=std::string("out"); // 离开交易池标记
        // record交易排队时延[交易离开交易池]
        stringstream ss;
        ss<<measure_time<<","<<tx_hash<<","<<flag<<"\n";
        std::shared_ptr<RecorderFile> recorderfile(new RecorderFile());
        recorderfile->Record(ss.str(),"tx_queue_delay");
...
bool TxPool::insert(Transaction::Ptr _tx)
{
    ...
    auto measure_time=dev::getFormattedMeasureTime(); // 测量时间
    // record交易延迟[交易进入交易池]
    stringstream ss_tx_delay_start;
    ss_tx_delay_start<<measure_time<<","<<tx_hash<<"\n";
    std::shared_ptr<RecorderFile> recorderfile_tx_delay_start(new RecorderFile());
    recorderfile_tx_delay_start->Record(ss_tx_delay_start.str(),"tx_delay_start");

    auto flag=std::string("in"); // 进入交易池标记
    // record交易排队时延[进入交易池]
    stringstream ss_tx_queue_delay;
    ss_tx_queue_delay<<measure_time<<","<<tx_hash<<","<<flag<<"\n";
    std::shared_ptr<RecorderFile> recorderfile_tx_queue_delay(new RecorderFile());
    recorderfile_tx_queue_delay->Record(ss_tx_queue_delay.str(),"tx_queue_delay");
    // -------------

    m_txsHash[tx_hash] = p_tx; // 建立交易哈希->队列指针的映射
    return true;
}
